...
def accumulated_indexed_mul_segment(
    input1, 
    input2, 
    index1=None, 
    index2=None, 
    ...
    return indexed_mul_scale_gather(
        ...
        seg=seg,
        out=out,
        out_accumulated=True,  # 启用累加模式
        block_size_n=block_size_n,
        block_size_c=block_size_c,
        num_stages=num_stages
    )

def accumulated_indexed_outer_segment(
    input1, 
    input2, 
    index1=None, 
    index2=None, 
    seg=None, 
    out=None,
    block_size_n=8,  # 外积累加需要更小block_size_n
    block_size_c1=32, 
    block_size_c2=32, 
    num_stages=2
):
    r"""分段累加的索引外积（必须提供seg且至少一个索引）"""
    assert seg is not None, "seg cannot be None for segment operations"
    assert index1 is not None or index2 is not None, "At least one of index1 or index2 must be provided"
...
def accumulated_indexed_inner_segment(
    input1, 
    input2, 
    index1=None, 
    index2=None, 
...
def accumulated_indexed_vecmat_segment(
    input1, 
    input2, 
    index1=None, 
    index2=None, 
    ...
    r"""分段累加的索引向量-矩阵乘（必须提供seg且至少一个索引）"""
    assert seg is not None, "seg cannot be None for segment operations"
    assert index1 is not None or index2 is not None, "At least one of index1 or index2 must be provided"
...
def accumulated_indexed_vecsca_segment(
    input1, 
    input2, 
    index1=None, 
    index2=None, 
