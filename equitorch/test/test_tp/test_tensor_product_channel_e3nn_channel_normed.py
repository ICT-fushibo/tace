...
def e3nn_tp_weights_to_eqt(tp_e3nn, tp_eqt, weight_e3nn):
    ...
    for ins_idx, ins in enumerate(tp_e3nn.instructions):
        ...
        if tp_eqt.path_norm:
            ...
            scale = ins.path_weight  / irrep_out[ins.i_out][1].dim ** (0.5) * (fan_in)**(0.5) 
...
def e3nn_tp_weights_grad_to_eqt(tp_e3nn, tp_eqt, weight_grad_e3nn):
    ...
    for ins_idx, ins in enumerate(tp_e3nn.instructions):
        ...
        if tp_eqt.path_norm:
            ...
            inv_scale = ins.path_weight  / irrep_out[ins.i_out][1].dim ** (0.5) * (fan_in)**(0.5) 
...
if __name__ == '__main__':
    ...
    irreps_str1 = '+'.join(f'1x{l}e' for l in range(L_max + 1))
    irreps_str2 = '+'.join(f'1x{l}e' for l in range(L_max + 1)) 
    irreps_str_out = '+'.join(f'1x{l}e' for l in range(L_max + 1))
    # irreps_str1 = '1x0e + 1x1o'
    # irreps_str2 = '1x0e + 1x1o'
