...
class FunctionTester:
    ...
    def profile(self, repeat: int = 1, print_prof=True, trace_name_func: Optional[Callable] = None):
        """
        对所有函数进行性能分析
        ...
        for func_name, (func, args, kwargs) in self.funcs_to_test.items():
            try:
                profs[func_name] = self._profile_func(
                    func, 
                    func_name, 
    ...
    def _profile_func(self, func: Callable, func_name: str, trace_name: Optional[str], 
                     repeat: int, print_prof:bool, *args, **kwargs):
        """
        内部方法: 分析单个函数的性能
        """
        activities = [
    ...
    def timeit(self, repeat: int = 1, warmup: int = 0, synchronize: bool = True) -> Dict[str, float]:
        """
        简单计时测试
        ...
        for func_name, (func, args, kwargs) in self.funcs_to_test.items():
            ...
            timings[func_name] = avg_time * 1000
        
        return timings 

    def multi_timeit(self, repeat: int = 1, warmup: int = 0, synchronize: bool = True, runs: int = 1) -> Dict[str, Dict[str, float]]:
        """
        计时测试，支持多次运行并计算统计信息
        
        参数:
            repeat: 每次运行中每个函数执行的次数
            warmup: 每次运行前的预热次数
            synchronize: 是否同步CUDA操作
            runs: 完整测试运行的次数
