...
def _init_test_case(irreps, channels, batch_size, device, affine, scaled, training):
    ...
    layer.train(training)  # Set training mode
...
def test_norm_case(irreps, channels, batch_size, affine, scaled, training):
    r"""Test normalization with given parameters."""
    print(f"\nTesting BatchRMSNorm with irreps={irreps}, channels={channels}, "
          f"batch_size={batch_size}, affine={affine}, scaled={scaled}, training={training}")
    
    layer, x = _init_test_case(irreps, channels, batch_size, device, affine, scaled, training)
    gradcheck_success, gradgradcheck_success = _run_gradcheck(layer, x)
    
    print(f"BatchRMSNorm - gradcheck: {gradcheck_success}, gradgradcheck: {gradgradcheck_success}")
    assert gradcheck_success and gradgradcheck_success

# Main execution
if __name__ == '__main__':
    print("Running gradient checks for BatchRMSNorm...")
    
    # Test different combinations of affine, scaled and training
    test_configs = [
        ...
        for affine in [True, False]:
            for scaled in [True, False]:
                for training in [True, False]:
                    test_norm_case(irreps, channels, batch_size, affine, scaled, training)
