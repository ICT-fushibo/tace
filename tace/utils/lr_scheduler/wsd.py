"""
To avoid dependency, copy from https://github.com/kozistr/pytorch_optimizer/blob/main/pytorch_optimizer/lr_scheduler/wsd.py
...
def get_1square_cooldown_lr_ratio(
    ...
    return 1.0 - math.pow(
        (current_step - num_warmup_steps - num_stable_steps) / num_decay_steps, 2
    )
    ...
    *,
    ...
    if current_step < num_warmup_steps + num_stable_steps + num_decay_steps:
        ...
        if cooldown_type == "1-sqrt":
            return get_1sqrt_cooldown_lr_ratio(
                current_step, num_warmup_steps, num_stable_steps, num_decay_steps
            )
        if cooldown_type == "1-square":
            return get_1square_cooldown_lr_ratio(
                current_step, num_warmup_steps, num_stable_steps, num_decay_steps
            )
        if cooldown_type == "linear":
            return get_linear_cooldown_lr_ratio(
                current_step, num_warmup_steps, num_stable_steps, num_decay_steps
            )
    return min_lr_ratio
