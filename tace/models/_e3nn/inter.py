...
from .fused import O3ScatterTensorProduct, SO2ScatterTensorProduct, SO2Attention
...
class SO2Interaction(Interaction):
    def _setup(self) -> None:
        ...
        self.rejector = SO2ScatterTensorProduct(
            ...
            separate_so2_radial=self.separate_so2_radial,
            num_head=self.num_head,
            use_graph_softmax=self.use_graph_softmax,
            use_so2_edge_ace=self.use_so2_edge_ace,
        ...
        if self.separate_so2_radial:
            assert self.rejector.weight_numel % 2 == 0
            self.edge_info = FFN[self.edge_info_type](
                [self.num_radial_basis] + self.radial_mlp + [self.rejector.weight_numel // 2],
                bias=self.radial_bias,
                layer_norm=self.radial_layer_norm,
                act=self.radial_act,
            )
            self.edge_info2 = FFN[self.edge_info_type](
                [self.edge_feats_channel - self.num_radial_basis] + self.radial_mlp + [self.rejector.weight_numel // 2],
                bias=self.radial_bias,
                layer_norm=self.radial_layer_norm,
                act=self.radial_act,
            )
            with torch.no_grad():
                self.edge_info.mlp[-1].weight.data /= math.sqrt(2.0)
                self.edge_info2.mlp[-1].weight.data /= math.sqrt(2.0)
        else:
            self.edge_info = FFN[self.edge_info_type](
                [self.edge_feats_channel] + self.radial_mlp + [self.rejector.weight_numel],
                bias=self.radial_bias,
    ...
    def forward(
        ...
        if self.separate_so2_radial:
            conv_weights = self.edge_info(edge_feats[0])
            conv_weights2 = self.edge_info2(edge_feats[1])
            conv_weights = torch.stack([conv_weights, conv_weights2], -1).view(conv_weights.size(0), -1)
        else:
            conv_weights = self.edge_info(edge_feats)
