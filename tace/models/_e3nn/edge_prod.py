...
class ComplexProductBasisV1(torch.nn.Module):
    def __init__(
        self,
        mmax: int,
        lmax: int,
...
class ComplexProductBasisV2(torch.nn.Module):
    def __init__(
        ...
        self.lmax = lmax
        self.num_channel = num_channel
        self.num_components = (lmax + 1) + sum(
            2 * (lmax + 1 - m) for m in range(1, mmax + 1)
        )
        self.num_channelwise_components = (lmax + 1) * (1 + 2 * mmax)
        self.tp = ComplexProductBasisV1(
            mmax=mmax,
            lmax=lmax,
        ...
        channelwise_indices = []
        ordinary_offset = 0
        channelwise_offset = 0
        for m in range(mmax + 1):
            n = lmax + 1 - m
            copies = 1 if m == 0 else 2
            for copy in range(copies):
                ordinary_indices.extend(
                    range(ordinary_offset + copy * n, ordinary_offset + (copy + 1) * n)
                )
                padded_start = channelwise_offset + copy * (lmax + 1) + m
                channelwise_indices.extend(range(padded_start, padded_start + n))
            ordinary_offset += copies * n
            channelwise_offset += copies * (lmax + 1)

        self.register_buffer(
            "ordinary_indices",
            torch.tensor(ordinary_indices, dtype=torch.long),
            persistent=False,
        )
        self.register_buffer(
            "channelwise_indices",
            torch.tensor(channelwise_indices, dtype=torch.long),
            persistent=False,
        )

    def _to_channelwise(self, x: torch.Tensor) -> torch.Tensor:
        # if x.shape[1:] != (self.num_components, self.num_channel):
        #     raise ValueError(
        #         "ComplexProductBasisV2 expected input shape "
        #         f"[batch, {self.num_components}, {self.num_channel}], got {tuple(x.shape)}"
        #     )
        out = x.new_zeros(
            x.shape[0], self.num_channelwise_components, self.num_channel
        )
        out[:, self.channelwise_indices] = x[:, self.ordinary_indices]
        return out

    def _from_channelwise(self, x: torch.Tensor) -> torch.Tensor:
        return x[:, self.channelwise_indices]
    ...
    def forward(
        ...
        channelwise = self._to_channelwise(x)
        channelwise = self.tp(channelwise, node_attrs, edge_index)
        return self._from_channelwise(channelwise)
