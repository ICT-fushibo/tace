...
def _to_full_so3_irreps(lmax: int | List[int]) -> List[int]:
    if isinstance(lmax, int):
        return list(range(0, lmax+1))
...
class Interaction(torch.nn.Module):
    def __init__(
        ...
        assert ictp_ictc_like, "cartTACE must use ictp_ictc_like = True"
    ...
    def truncate_ghosts(
        self, tensor: Optional[torch.Tensor| Dict[int, torch.Tensor]], nlocal: Optional[int] = None
    ) -> torch.Tensor:
        if tensor is None:
            return tensor
