    ...
    def __init__(self, comb: Tuple) -> None:
        ...
        self.ctr = (
            opt_einsum_fx.optimize_einsums_full(
                model=ctr,
                example_inputs=(
                    torch.randn([BATCH_] + [3] * l1 + [CHANNEL_]),
                    torch.randn([BATCH_] + [3] * l2 + [CHANNEL_]),
                ),
            )
        )
        self.alpha = 1.0 / sqrt(3**k)

    def forward(self, T1: torch.Tensor, T2: torch.Tensor) -> torch.Tensor:
        return self.ctr(T1, T2) * self.alpha
    ...
    def __init__(self, comb: Tuple) -> None:
        ...
        self.ctr = (
            opt_einsum_fx.optimize_einsums_full(
                model=ctr,
                example_inputs=(
                    torch.randn([PATH_] + [BATCH_] + [3] * l1 + [CHANNEL_]),
                    torch.randn([BATCH_] + [3] * l2 + [CHANNEL_]),
                ),
            )
        )
