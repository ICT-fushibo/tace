...
def mh_mask(
        x: torch.Tensor, node_fidelity: torch.Tensor, num_fidelities: int
    ) -> torch.Tensor:
    B = x.size(0)
    C_ALL = x.size(1)
...
class ScalarReadOut(ReadOut):
    ...
    def forward(
        self, x: Dict[int, torch.Tensor], node_fidelity: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        if not self.last_layer:
            return self.linear1(x)[0]
        x = self.act(self.linear1(x)[0])
        if self.num_fidelities > 1:
            x = mh_mask(x, node_fidelity, self.num_fidelities)
        return self.linear2({0: x})[0]
