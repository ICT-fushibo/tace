...
class cartTACE(torch.nn.Module):
    def __init__(
        self,
        Lmax: int,
        lmax: int,
        ...
        if "energy" in self.target_property:
            ...
            for ii, energy_readout in enumerate(self.energy_readouts):
                if not self.use_alllayer:
                    ii = -1
                e_list.append(energy_readout(descriptors[ii], node_fidelity)[num_atoms_arange, node_fidelity])   
    ...
    def forward(self, data: Dict[str, torch.Tensor], graph: Graph) -> Dict[str, Any]:
        outs = self.representation(data, graph)
