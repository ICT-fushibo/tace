...
class SpectralACE(Product):
    ...
    def _setup(self):
        ...
        for nu in range(2, self.correlation+1):
            path, irreps_out, _, _ = generate_eqt_e3nn_paths(
            ...
            this_ace = TensorProduct(
                irreps_in1=product_in1,
                irreps_in2=self.irreps_in,
                irreps_out=irreps_out,
                channels_in1=self.num_channel,
                channels_in2=self.num_channel,
            ...
            self.coefs.append(
                ElementLinear(
                    this_ace.irreps_out.simplified(),
                    self.irreps_out,
                    self.num_channel,
                    self.num_hidden_channel,
                    bias=self.use_bias,
...
class SpatialACE(Product):
    ...
    def _setup(self):
        ...
        ls_in = [ir.l for ir in self.irreps_in]      
        ls_out = [ir.l for ir in self.irreps_out]
        out_slices = []
        sum_l = 0
        start = 0
        for l in ls_out:
