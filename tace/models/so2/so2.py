...
class SO2Norm(torch.nn.Module):
    ...
    def __init__(
        ...
        class ComplexSigmoid(torch.nn.Module):
            ...
            def forward(self, x: torch.Tensor) ->  torch.Tensor:
                magnitude = (x**2).sum(dim=1, keepdim=True) * 0.5
                magnitude = self.act(magnitude)
                return x * magnitude
    
        self.real_activation = ScaledSiLU()
        self.complex_activation = ComplexSigmoid()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # m = 0
        x0 = xs[0]
        x0 = self.real_activation(x0)
        out.append(x0)
        # m > 0
        for xm in xs[1:]:
            xm = self.complex_activation(xm)
            out.append(xm)
