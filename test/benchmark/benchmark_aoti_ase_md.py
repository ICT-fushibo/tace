...
################################################################################
"""Benchmark eager PyTorch and AOTI with a fixed 192-atom ASE MD system.

Compile once from the repository root on the second GPU:
...
Run the benchmark:
    ...
    python test/benchmark/benchmark_aoti_ase_md.py
"""
...
def build_calculator(model_path: Path) -> TACEAseCalc:
...
def run_md(atoms, calculator: TACEAseCalc, steps: int):
    state = atoms.copy()
    ...
    dynamics = MelchionnaNPT(
        ...
        ttime=THERMOSTAT_TIMESCALE_FS * units.fs,
        pfactor=None,
        externalstress=0.0,
        logfile=None,
    )
...
def check_outputs(atoms, eager_calculator, aoti_calculator) -> tuple[float, float]:
    eager_atoms = atoms.copy()
    eager_atoms.calc = eager_calculator
    eager_energy = eager_atoms.get_potential_energy()
    eager_forces = eager_atoms.get_forces()

    aoti_atoms = atoms.copy()
    aoti_atoms.calc = aoti_calculator
    aoti_energy = aoti_atoms.get_potential_energy()
    aoti_forces = aoti_atoms.get_forces()

    energy_error = abs(eager_energy - aoti_energy)
    force_error = float(np.max(np.abs(eager_forces - aoti_forces)))
    if not np.isclose(eager_energy, aoti_energy, atol=ATOL, rtol=RTOL):
        raise RuntimeError(
            f"Eager and AOTI energies differ: {eager_energy} vs {aoti_energy}"
        )
    if not np.allclose(eager_forces, aoti_forces, atol=ATOL, rtol=RTOL):
        raise RuntimeError(
...
def benchmark(atoms, calculator: TACEAseCalc) -> Sequence[float]:
    if WARMUP_STEPS:
